...
import subprocess
from OmniSR import OmniSR  # Assuming this is how you import the OmniSR class
import sys
import os
# Load the pretrained model using the provided state_dict
def vitogiff(input_image, output):
    script_directory = os.path.dirname(os.path.realpath(__file__))
    state_dict = torch.load(script_directory+'/epoch994_OmniSR.pth', map_location='cpu')  # Load on CPU
    model = OmniSR(state_dict)
    # Check if either height or width is greater than 3000 pixels
    if input_image.height >= 1800 or input_image.width >= 1800:
        ...
        final_width = input_image.width * 4
        final_height = input_image.height * 4
...
def vito(input_filename, type):
    os.chdir("C:/Users/91741/AppData/Local/pixelperfect/app-1.0.0/resources/python")
    command = f'realesrgan-ncnn-vulkan.exe -i "{input_filename}" -o "{type}" -n realesrgan-x4plus'
    print(command)
